...
window.WebCLCommon = (function (debug) {
    ...
    /* Global vars */
    var platforms = [], devices = [], context = null, program = null, deviceType = null, platform = null;
    ...
    return {
        ...
        init : function (type) {
            ...
            for (i = 0; i < platforms.length; i++) {
                switch (type) {
                case "CPU":
                    dev = platforms[i].getDevices();
                    for (var j=0; j<dev.length; j++) {
                        if (dev[j].getInfo(webcl.DEVICE_TYPE) == webcl.DEVICE_TYPE_CPU) {
                            addElementsFromList([dev[j]], devices);
                ...
                case "GPU":
                    dev = platforms[i].getDevices();
                    for (var j=0; j<dev.length; j++) {
                        if (dev[j].getInfo(webcl.DEVICE_TYPE) == webcl.DEVICE_TYPE_GPU) {
                            addElementsFromList([dev[j]], devices);
                            platform = platforms[i];
...
// 
            ...
            context = webcl.createContext({platform: platform, devices: devices, deviceType: deviceType});
            return context;
        },
