function JAWS(nCPU) {
    ...
    var OPT = false;                 // true: dynamic chunk size, false: static chunk size;
    ...
    function finishJob() {
        ...
        console.log("CPU works: " + CPU.time + " ms (" + CPU.done + " elements)");
        console.log("GPU works: " + GPU.time + " ms (" + GPU.done + " elements)");
        cpuExe += CPU.time;
        gpuExe += GPU.time;
...
/************ static optimal config start ***********/
		if(i == 0)
            ...
            chunkSize = 512; // change this number for correlation
/************ static optimal config end ***********/
        ...
        GPU.chunkSize = getChunkSize(DATA_SIZE - chunkSize);
        if (((DATA_SIZE - CPU.chunkSize) < GPU.chunkSize))
            GPU.chunkSize = DATA_SIZE - CPU.chunkSize
