function JAWS(nCPU) {
    ...
    /* Attribute */
    this.initRate = 0.07;          // intial chunks
    this.merge = [0];
    this.SIMD = false;
    this.ext = null;
    this.debug = false;
    ...
    var CPU = new function() {
        ...
        function onMsg(e) {
            ...
            if (wait == numCPU) {
				...
				if (opt) {
                    if(GPU.count >= 2 && CPU.count >= 2){
                    	opt = false;
                        var partition = parseInt((DATA_SIZE - offset) * GPU.thruput / (CPU.thruput + GPU.thruput) - (GPU.chunkStart-GPU.scheduler.Time[GPU.scheduler.length-1])*GPU.thruput);
                        if(partition > 0){
    ...
    var GPU = new function() {
        ...
        function onMsg(e) {
			...
			if (opt) {
                    if(GPU.count >= 2 && CPU.count >= 2){
                    	opt = false;
                        var partition = parseInt((DATA_SIZE - offset) * GPU.thruput / (CPU.thruput + GPU.thruput) + (CPU.chunkStart-CPU.scheduler.Time[CPU.scheduler.length-1])*CPU.thruput);
                        if(partition > 0){
                        	GPU.chunkSize = partition;
                            GPU.chunkSize = getChunkSize(GPU.chunkSize); 
                       	}
						...
						console.debug("ggggg");
                        console.debug("DATA_SIZE - offset: "+(DATA_SIZE - offset));
                        console.debug("CPU.thruput:  "+CPU.thruput);
                        console.debug("GPU.thruput:  "+GPU.thruput);
                        console.debug("CPU.chunkSize:  "+CPU.chunkSize);
                        console.debug("GPU.chunkSize:  "+GPU.chunkSize);
                    } 
                    else {
