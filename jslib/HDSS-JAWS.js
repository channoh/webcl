function JAWS(nCPU) {
	...
	function Scheduler() {
        ...
        this.push = function(thruput, time, chunkSize) {
        ...
        this.getSlope = function() {
            ...
            if(len >= WindowSize){
                var slope = (this.Thruput[len-1]-this.Thruput[len-2]) / (this.Time[len-1]-this.Time[len-2]);
		...
		this.getLogBase = function() {
            var len = this.Thruput.length;
			this.base = Math.abs((this.Thruput[len-1]-this.Thruput[0]) / (Math.log(this.ChunkSize[len-1])-Math.log(this.ChunkSize[0])));
			debug("base: "+this.base);
    ...
    var CPU = new function() {
        ...
        function onMsg(e) {
            ...
            if (wait == numCPU) {
                ...
                CPU.scheduler.push(CPU.thruput, end, chunkSize);

                if (opt) {
            		var len = CPU.scheduler.Thruput.length;
                    if(GPU.thruput != 0 && (DATA_SIZE - offset) > DATA_SIZE*0.1 && len >= 4){
						if(CPU.scheduler.getLogBase() != null){
							CPU.chunkSize = CPU.scheduler.base/0.01;
							debug("CPU.chunkSize:  "+CPU.chunkSize);
						}
                        var slope1 = CPU.scheduler.getSlope();
                        var slope2 = GPU.scheduler.getSlope();
                    	if ((slope1 > minSlope && slope1 < maxSlope && slope2 > minSlope && slope2 < maxSlope) || done >= 0.2*DATA_SIZE) {
							opt = false;
    ...
    var GPU = new function() {
        ...
        function onMsg(e) {
            ...
            GPU.scheduler.push(GPU.thruput, end, chunkSize);

            if (opt) {
            	var len = GPU.scheduler.Thruput.length;
				if(CPU.thruput != 0 && (DATA_SIZE - offset) > DATA_SIZE*0.1 && len >= 4){
					if(GPU.scheduler.getLogBase() != null){
						GPU.chunkSize = GPU.scheduler.base/0.01;
						debug("GPU.chunkSize:  "+GPU.chunkSize);
					}
                    var slope1 = GPU.scheduler.getSlope();
                    var slope2 = CPU.scheduler.getSlope();
                    if ((slope1 > minSlope && slope1 < maxSlope && slope2 > minSlope && slope2 < maxSlope) || done >= 0.2*DATA_SIZE) {
                        opt = false;
                        var partition = parseInt((DATA_SIZE - offset) * (GPU.thruput / (CPU.thruput + GPU.thruput)));
                        if(partition > 0){
    ...
    function finishJob() {
        ...
        if (ext != null) {
            ...
            console.log("gpuExe: " + gpuExe + " ms");
            console.log("cpuEnd: " + cpuEnd + " ms");
            console.log("gpuEnd: " + gpuEnd + " ms");
            ext();
        }
        job.complete++;
        if (job.index >= job.complete) {
...
//			if ((Math.max(CPU.thruput, GPU.thruput) > Math.min(CPU.thruput, GPU.thruput) * 10) && balanced)
            ...
            debug(" * ratio: " + GPU.thruput / (CPU.thruput + GPU.thruput));
            debug(" * factor: " + factor);
            debug(" * CPU.slope: " + CPU.scheduler.getSlope());
            debug(" * GPU.slope: " + GPU.scheduler.getSlope());
        ...
        debug("init GPU chunkSize: " + GPU.chunkSize);
